...
rl.on('line', line => {
    ...
    GAMES.push({gameNumber: gameNo, pulls: pulls});
});
rl.on("close", () => {
    console.log(GAMES.filter(g => g.pulls.every(p => p.blueCount <= BLUE_COUNT && p.redCount <= RED_COUNT && p.greenCount <= GREEN_COUNT))
        .reduce((total, next) => total + next.gameNumber, 0));
});
